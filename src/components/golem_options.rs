...
pub struct Props {
    pub paused: bool,
    pub onstartstopclick: Callback<Golem>,
    pub onstepclick: Callback<Golem>,
}

pub struct GolemOptions {
    props: Props,
}

impl Component for GolemOptions {
    type Message = ();
    ...
    fn view(&self) -> Html {
        let start_button_value = if self.props.paused { "Start" } else { "Pause" };
        let step_button_disabled = !self.props.paused;

        html! {
            <div class="golem-options">
                <div class="golem-options-buttons">
                    <input type="button" class="golem-input-button" id="golem-options-start" value=start_button_value onclick=self.props.onstartstopclick />
                    <input type="button" class="golem-input-button" id="golem-options-step" value="Step" disabled=step_button_disabled onclick=self.props.onstepclick />
                ...
                <label class="golem-options-label" for="golem-options-presets">
                    ...
                    <select class="golem-input-select" id="golem-options-presets">
                        ...
                        <option value="23/3/2" selected=true>{ "Life" }</option>
